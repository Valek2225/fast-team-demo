...
const MARKETING_TERMS: Term[] = [
  ...
  },
  {
    ...
    inProject: 'Маскот и механика серии повышают retention. Без них D30 retention был бы ~8%, с серией ожидаем 18–25% — это в 2–3 раза больше LTV.',
  },
  {
    name: 'A/B-тест',
    alias: 'сплит-тест',
...
const METRIC_CASES: MetricCase[] = [
  {
    ...
    whatTheyDo:
      'Маскот ведёт серию и шлёт «настойчивые» пуши при пропуске. User flow построен вокруг ежедневной короткой сессии. Серия — главный visual cue в приложении.',
    northStar: {
    ...
    counterMetrics: [
      ...
      },
      {
        ...
        description:
          'Сколько процентов вернулись после потери серии. Если низкий — серийная механика выгнала больше, чем удержала. Точное значение Duolingo не раскрывает, но факт мониторинга подтверждён публикациями.',
      },
    ],
    takeawayForUs:
      'Серия отлично работает, но требует защитных клапанов: грейс-период, мягкий тон пушей, мониторинг соотношения «рост vs ощущение давления». В Т-Городе мы не должны ассоциироваться с тревогой.',
    sources: [
      {
  ...
  },
  {
    ...
    whatTheyDo:
      'Stripped-down маскот — это сама механика серии между друзьями: огонёк держится, пока обмениваетесь снэпами каждый день. Не персонаж, а статус.',
    northStar: {
    ...
    counterMetrics: [
      ...
      },
      {
        ...
        description:
          'Внутренний риск, который Snap признаёт — серия воспринимается как обязанность, особенно среди подростков. Точные числа не публикуются.',
      },
    ],
    takeawayForUs:
      'Механика серии без маскота тоже работает. Для Т-Города — Лис Толя становится «лицом» серии, но сама механика должна быть простой: 1 заказ в день = огонёк жив.',
    disclaimer:
  ...
  },
  {
    ...
    takeawayForUs:
      'Маскот не обязан давить. Толя может быть «помощником в успехе» — поздравлять с первым заказом, при серии ≥7 давать подарок. Метрика времени до первого заказа — наш aha moment.',
    disclaimer:
...
const COMPETITOR_CASES: CompetitorCase[] = [
  ...
  },
  {
    ...
    region: 'global',
    whatTheyDid:
      'Постоянный маскот с сезонными «скинами» (Halloween, Christmas, Valentine), которые меняют визуал, не ломая идентичность. На уровне маркетинга — viral memes («unhinged Duo» через TikTok), на уровне продукта — серия с push-механикой, Friend Streaks, групповые challenge.',
    metrics: [
      ...
      { label: 'Юзеров с серией ≥1 года (Q4 2024)', value: '> 10 млн' },
        ...
        <Stack gap={20}>
          <Stack gap={8}>
            ...
            <Text tone="secondary">
              ...
              какие KPI используют продукты с маскотами и серийными механиками. Для каждого кейса:
