...
- `/flow` — интерактивный сквозной сценарий внутри одного iPhone: корзина →
  ...
  FSM из 9 шагов, есть контролы «Сначала / Шаг ← / Шаг →» для удобной
  демонстрации команде. После первого стрика возврат «назад» из полной ленты
  ведёт в лобби **без повтора** авто-скролла и туториала к «Все»; в шапке
  остаётся плашка 🔥. Меню Толи: только контент + «?» (правила), карусель
  как в лобби; нижний sheet со скруглением под рамку телефона.

Смоук E2E (Playwright): `npm run test:e2e` (поднимает Vite на `127.0.0.1:5180`).
