...
│   └── favicon.svg
└── src/
    ├── main.tsx            # mount React + роутинг (/ vs /mobile vs /flow)
    ├── App.tsx             # вся презентация (главная страница)
    ├── FlowDemo.tsx        # /mobile — интерактивный сквозной сценарий (основной demo-route)
    ├── MobileDemo.tsx      # /flow — legacy статичный мокап двух экранов
...
- `/mobile` — интерактивный сквозной сценарий внутри одного iPhone: корзина →
  заказ → знакомство → главное лобби (с авто-скроллом и подсказкой) → ищем
  товар Толи в карусели → серый оверлей с акцентом → success «Стрик +1».
  FSM из шагов; контролы «Сначала / Шаг ← / Шаг →» скрыты для демо жюри,
  но остаются в DOM — для Playwright поднимается Vite с `--mode test` и
  `.env.test` (`VITE_SHOW_FLOW_CONTROLS=1`), чтобы кнопки были видимы в E2E.
  После первого стрика возврат «назад» из полной ленты ведёт в лобби **без повтора**
  авто-скролла и туториала к «Все»; в шапке остаётся плашка 🔥. Меню Толи:
  только контент + «?» (правила), карусель как в лобби; нижний sheet под рамку телефона.
- `/flow` — legacy: два статичных экрана iPhone (Заказ оформлен → Знакомство с Лисом Толей).

Смоук E2E (Playwright): `npm run test:e2e` (Vite на `127.0.0.1:5180`, режим `test`).
